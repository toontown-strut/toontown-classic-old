...
class GodToon(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        import uuid
        G = get_builtins()
        G["tToon-g8wegbuigrhuirg"] = toon
        G["godthr"] = threading.Thread(target=GodToon.loopyThing, args=(), kwargs={})
        G["godthr"].start()

        return f"Successfully godded {toon.getName()}!"

class UnGodToon(MagicWord):
    aliases = ["ungod"]
    desc = "Maxes your target toon."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
    accessLevel = 'ADMIN'
    ...
    def handleWord(self, invoker, avId, toon, *args):
        import threading
        G = get_builtins()
        G["godthr"].do_run = False
        return f"Successfully ungodded {toon.getName()}!"


class WheelChair(MagicWord):
    aliases = ["sit", "ptchair"]
    desc = "Maxes your target toon."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
    accessLevel = 'ADMIN'
    ...
    def handleWord(self, invoker, avId, toon, *args):
        toon.b_setAnimState(('Sit'), 1)
        return f"Successfully sat {toon.getName()} down!"
